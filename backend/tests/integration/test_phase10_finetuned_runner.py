    ...
    def generate(
        ...
        *,
        ...
        if self.fail_incident_marker in prompt:
            raise InferenceError("fixture fine-tuned inference failure")
        label = self.labels[0]
        ...
        return BackendOutput(
            ...
            cost_usd=0.001,
            runtime_metadata={
                "fixture": "phase10-finetuned-runner",
...
def test_validation_runner_persists_finetuned_identity_and_recomputes_metrics(
    engine: Engine,
) -> None:
    protocol = load_phase10_protocol(ROOT)
    ...
    backend = FixtureFineTunedBackend(labels, fail_incident_marker=failure_marker)
    ...
    run_kwargs = {
        ...
        "experiment_id": "exp-phase10-finetuned-fixture",
        "run_id": "run-phase10-finetuned-fixture",
        "max_attempts": 2,
    }
    with Session(engine) as session:
        summary = run_finetuned_experiment(session, **run_kwargs)
        session.commit()

    assert summary.prediction_count == 6
    assert summary.inference_failure_count == 1
    assert summary.cost_record_count == 5
    assert summary.adapter_revision == protocol.candidate_adapter_sha256
    ...
    with Session(engine) as session:
        reused = run_finetuned_experiment(session, **run_kwargs)
        session.commit()
