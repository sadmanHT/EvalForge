...
def _assert_equal(actual: object, expected: object, message: str) -> None:
    if actual != expected:
        raise SystemExit(f"PHASE04_EXIT_GATE=FAIL {message}: actual={actual!r} expected={expected!r}")
...
def main() -> int:
    ...
    with psycopg.connect(Settings.from_env().database_url) as connection:
        with connection.cursor() as cursor:
            ...
            _assert_equal(dataset_row[0], dataset["manifest_checksum"], "manifest checksum mismatch")
            ...
            if missing_triggers:
                raise SystemExit(
                    f"PHASE04_EXIT_GATE=FAIL historical immutability triggers missing={missing_triggers}"
                )
